...
# Yeni bir date değişkeni oluşturma
df['date'] = pd.to_datetime(df['CreateDate']).dt.to_period('M')

# UserID ve date değişkenlerini birleştirerek yeni bir ID değişkeni oluşturma
...
pivot = df.groupby(["ID","Service_Category"])["Service_Category"].count().unstack().fillna(0).applymap(lambda x:1 if x > 0 else 0)
pivot.head()

# Adım 2: Birliktelik kurallarını oluşturunuz.

frequent_itemsets = apriori(pivot, min_support=0.005, use_colnames=True)
